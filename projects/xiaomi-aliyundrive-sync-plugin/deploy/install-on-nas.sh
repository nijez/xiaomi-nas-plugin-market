...
ssh "${SSH_OPTIONS[@]}" "${REMOTE_TARGET}" "sed 's|__NAS_USER_ID__|${NAS_USER_ID}|g' /tmp/xiaomi-aliyundrive-sync.nginx.conf > /etc/nginx/conf.d/luci/xiaomi-aliyundrive-sync.conf.new && nginx -t && mv /etc/nginx/conf.d/luci/xiaomi-aliyundrive-sync.conf.new /etc/nginx/conf.d/luci/xiaomi-aliyundrive-sync.conf"
...
ssh "${SSH_OPTIONS[@]}" "${REMOTE_TARGET}" "systemctl is-active --quiet xiaomi-aliyundrive-sync.service && python3 -c \"import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:18117/api/status', timeout=8).read().decode())\""
