#!/usr/bin/env python3
"""Register only the Jingsu ERP link in a Xiaomi NAS user's plugin list."""
...
def plugin_record(plugin_id: int, now: int) -> dict[str, Any]:
    return {
        ...
        "frontend": {
            ...
            "dev_type": [1, 2, 3, 4],
            "url": [
                {"dev_type": [1], "url": "/index.html#/erp_app"},
                {"dev_type": [2, 3, 4], "url": "/index.html#/erp_pc"},
            ],
            "sortid": plugin_id,
            "widget": [],
        },
...
def write_registry(path: Path, registry: dict[str, Any]) -> None:
    temporary = path.with_suffix(path.suffix + ".erplink.tmp")
...
def main() -> int:
    ...
    parser.add_argument("--plugin-id", type=int, default=1003)
    ...
    if registry_path.exists():
        backup = registry_path.with_name(
            f"{registry_path.name}.before-erplink-{int(time.time())}.bak"
        )
...
if __name__ == "__main__":
    try:
        ...
        print(f"ERP link registration refused: {error}", file=sys.stderr)
        raise SystemExit(1)
