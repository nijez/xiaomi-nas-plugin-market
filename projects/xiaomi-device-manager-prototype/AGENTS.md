...
For the device-manager metrics, expand the selected metric's trend immediately beneath its card. On desktop, keep the paired metric card in the same row; on mobile, the trend must directly follow the tapped metric instead of appearing at the bottom of the page.
