...
此分享版本为 0.1.1 测试发布，非小米官方产品。Windows 实体安装尚未验收；115 和阿里云盘均需要使用者自己获批的 App ID，不能省略开发者应用授权。WebDAV 多用户版本在列表中标为测试版。
...
内置仓库包含设备管家、115 云备份、阿里云盘备份和 WebDAV 文件桥。WebDAV 文件桥自带经过官方 SHA-256 校验的 rclone ARM64 引擎，安装后共享默认关闭，远程连接需由用户填写自己的 WebDAV 账号。它不修改小米原有 5000 端口服务。
