...
此分支的 0.1.3 安装器是未发布的加固源码候选，非小米官方产品。现有签名包不包含本轮修复，不可重新打包当作修复版分享。Windows 参数传输夹具通过不等于 Windows 10/11 实机安装验收；Mac/Android 中继身份也尚未完成实机验证。115 和阿里云盘均需要使用者自己获批的 App ID。发布前置条件见 [加固进度](../../docs/HARDENING-20260905.md)。
...
以下是已验收发布包的目标流程，不是建议安装当前源码候选。当前受审 115 离线依赖尚未齐备，中继路径缺少可靠身份时会拒绝授权。
